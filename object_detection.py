import cv2
import numpy as np
import os

class ObjectDetection:
    def __init__(self, weights_path="dnn_model/yolov4.weights", cfg_path="dnn_model/yolov4.cfg"):
        print("Loading Object Detection")
        print("Running opencv dnn with YOLOv4")
        self.nmsThreshold = 0.4
        self.confThreshold = 0.5
        self.image_size = 608

        # Load Network
        net = cv2.dnn.readNet(weights_path, cfg_path)

        # Optionally enable CUDA if available
        try:
            net.setPreferableBackend(cv2.dnn.DNN_BACKEND_CUDA)
            net.setPreferableTarget(cv2.dnn.DNN_TARGET_CUDA)
        except Exception as e:
            print("CUDA is not available, using CPU:", e)
            net.setPreferableBackend(cv2.dnn.DNN_BACKEND_OPENCV)

        self.model = cv2.dnn_DetectionModel(net)
        self.classes = []
        self.load_class_names()
        self.colors = np.random.uniform(0, 255, size=(80, 3))
        self.model.setInputParams(size=(self.image_size, self.image_size), scale=1/255)

    def load_class_names(self, classes_path="dnn_model/classes.txt"):
        if not os.path.exists(classes_path):
            raise FileNotFoundError(f"Cannot find classes file at {classes_path}.")
        with open(classes_path, "r") as file_object:
            for class_name in file_object.readlines():
                self.classes.append(class_name.strip())

    def detect(self, frame):
        # Ensure this method is correctly defined and functioning
        # Adjust nmsThreshold and confThreshold as needed
        return self.model.detect(frame, nmsThreshold=self.nmsThreshold, confThreshold=self.confThreshold)
